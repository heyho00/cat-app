...
const ChatPage = () => {
  ...
  useEffect(() => {
    let timer;
    if (answer.length==2) {
      let messages = [`${answer[1]}!`, '기가 막히네 !', '영어가 좋아 한글이 좋아?'];
  ...
  useEffect(() => {
    let timer;
    if (answer.length==3) {
      let messages = [`알겠어, ${answer[2]}로 지어줄게!`, '마지막으로 반영하고싶은 고양이의 특징이 뭐니?'];
      if (displayedMessages4.length < messages.length && !!answer[0]) {
        timer = setTimeout(() => {
          setDisplayedMessages4(messages.slice(0, displayedMessages4.length + 1));
        }, 800);
      }
      return () => clearTimeout(timer);
    }
    ...
    <div className="flex flex-col items-center justify-center h-screen">
      <div className="bg-white p-8 rounded-lg shadow w-[30rem]">
        ...
        {displayedMessages4.length>=2 && !answer[3] &&
          <InputBox inputValue={inputValue} handleChange={handleChange} submit={submit} />
        }
      </div>
    </div>
    <div className="h-24" />
